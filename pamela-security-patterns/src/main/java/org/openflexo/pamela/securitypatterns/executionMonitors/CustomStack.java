...
/**
 * Execution monitor storing a call stack for all method handled by the ProxyMethodHandler. This monitor is used when there is a need for a
 * pattern to know previous calls (check a call sequence, check a calling instance or method, etc.)
 */
public class CustomStack extends ExecutionMonitor {
	...
	public CustomStack(PamelaModel model) {
		super(model);
	...
	private boolean isMonitored(Object instance) {
		// TODO: pref issue here !
		return !this.getModel().getMetaModel().getUpperEntities(instance).isEmpty();
	}
}
