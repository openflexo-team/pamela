...
public class SingleAccessPointPatternDefinition extends PatternDefinition {
		...
		for (ModelEntity<?> accessor : this.getAccessorEntities().keySet()) {
			...
			for (String paramID : this.checkpointParams.keySet()) {
				if (!wrapper.getGettersMap().containsKey(paramID)) {
					this.isValid = false;
					this.message += "Invalid SingleAccessPointClient entity (class " + accessor.getImplementedInterface().getSimpleName()
							+ ") for pattern " + this.getIdentifier() + ": missing RequiredForAccess annotation with paramID " + paramID
							+ ".\n";
				}
			}
		}
	...
	public <I> void notifiedNewInstance(I newInstance, ModelEntity<I> modelEntity, PamelaModel model) {
		if (modelEntity == this.systemEntity) {
			SingleAccessPointPatternInstance<I> patternInstance = new SingleAccessPointPatternInstance<I>(this, model, newInstance);
		}
	}
			...
			if (a != null && a.patternID().compareTo(this.getIdentifier()) == 0) {
				if (this.checkpoint == null) {
					...
					if (!boolean.class.isAssignableFrom(this.checkpoint.getReturnType())) {
						this.isValid = false;
						this.message += "Invalid Protected system entity for pattern " + this.getIdentifier()
								+ ": checkpoint method does not return a boolean.\n";
					}
					for (int i = 0; i < m.getParameterCount(); i++) {
						...
						if (r == null || r.patternID().compareTo(getIdentifier()) != 0) {
							this.isValid = false;
							message += "Invalid Protected system entity for pattern " + this.getIdentifier()
									+ ": missing @RequiredForAccess annotation for parameter #" + i + " of @Checkpoint method.\n";
						}
						else {
							if (!this.checkpointParams.containsKey(r.paramID())) {
								...
								this.isValid = false;
								this.message += "Invalid Protected system entity for pattern " + this.getIdentifier()
										+ ": duplicate @RequiredFoAccess annotation with same paramID " + r.paramID() + ".\n";
							}
						}
					}
				}
					...
					this.isValid = false;
					this.message += "Invalid Protected system entity for pattern " + this.getIdentifier()
							+ ": duplicate @Checkpoint annotation.\n";
				}
			}
		}
		if (this.checkpoint == null) {
			...
			for (Method m : accessorEntity.getImplementedInterface().getMethods()) {
				...
				if (r != null && r.patternID().compareTo(this.getIdentifier()) == 0) {
					if (!requiredGetters.containsKey(r.paramID())) {
						...
						this.isValid = false;
						this.message += "Invalid SingleAccessPointClient entity (class "
								+ accessorEntity.getImplementedInterface().getSimpleName() + ") for pattern " + this.getIdentifier()
								+ ": duplicate @RequiredForAccess with same paramId " + r.paramID() + ".\n";
					}
				}
			}
