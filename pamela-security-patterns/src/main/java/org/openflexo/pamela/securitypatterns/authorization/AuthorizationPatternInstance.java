 ...
 */
public class AuthorizationPatternInstance<S, R, C> extends PatternInstance<AuthorizationPatternDefinition>
	...
	public <I> AuthorizationPatternInstance(AuthorizationPatternDefinition patternDefinition, PamelaModel model) {
	...
	private void checkResourceIdIsFinal(R instance) {
		try {
			for (String paramID : this.getPatternDefinition().getResourceIdGetters().keySet()) {
				...
				if (this.resources.get(instance).getIdentifiers().get(paramID) != null
						&& !this.resources.get(instance).getIdentifiers().get(paramID).equals(currentID)) {
	...
	private ReturnWrapper processSubjectMethodBeforeInvoke(Object instance, Method method, Object[] args)
	...
	private void checkIdIsFinal(S instance) {
		...
		try {
			for (String s : getPatternDefinition().getSubjectIdGetters().keySet()) {
				...
				if (this.subjects.get(instance).getIdentifiers().get(s) != null
						&& !this.subjects.get(instance).getIdentifiers().get(s).equals(currentID)) {
	...
	private ReturnWrapper processSubjectAccess(Object instance, Method m, Object[] args)
			throws InvocationTargetException, IllegalAccessException {
		...
		if (allowed) {
			this.resources.get(resourceInstance).allowMethod(getPatternDefinition().getResourceAccessMethods()
					.get(getPatternDefinition().getSubjectAccessMethods().get(m).getMethodID()), getPatternDefinition());
	...
	private boolean performAccessCheck(Method m, Object instance, R resourceInstance)
			throws InvocationTargetException, IllegalAccessException {
		C checker = this.resources.get(resourceInstance).getChecker();
		int nbParam = this.getPatternDefinition().getResourceIdParameters().size() + getPatternDefinition().getSubjectIdParameters().size()
				+ 1;
		Object[] args = new Object[nbParam];
		for (String subId : getPatternDefinition().getSubjectIdParameters().keySet()) {
			args[getPatternDefinition().getSubjectIdParameters().get(subId)] = getPatternDefinition().getSubjectIdGetters().get(subId)
					.invoke(instance);
		}
		for (String resId : getPatternDefinition().getResourceIdParameters().keySet()) {
			args[getPatternDefinition().getResourceIdParameters().get(resId)] = getPatternDefinition().getResourceIdGetters().get(resId)
	...
	private ReturnWrapper processAllowedAccess(Method m, Object instance, R resourceInstance, Object[] args)
			throws InvocationTargetException, IllegalAccessException {
		Method resourceMethod = getPatternDefinition().getResourceAccessMethods()
				.get(getPatternDefinition().getSubjectAccessMethods().get(m).getMethodID());
