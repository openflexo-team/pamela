...
public class PermissionCheckerEntity {
		...
		if (this.checkMethod == null) {
			if (!boolean.class.isAssignableFrom(m.getReturnType())) {
				throw new InconsistentPermissionCheckerEntityException(
						String.format("Check method %s in class %s does not return a boolean.", m.getName(), this.baseClass));
			}
			this.checkMethod = m;
			for (int i = 0; i < m.getParameters().length; i++) {
				...
				if (subjectAnnotation != null && resourceAnnotation != null) {
					throw new InconsistentPermissionCheckerEntityException(
							"Parameter with both @SubjectID and @ResourceID annotations with same patternID " + this.pattern.getID()
									+ " in class " + this.baseClass.getSimpleName());
				}
				if (subjectAnnotation != null && subjectAnnotation.patternID().compareTo(this.pattern.getID()) == 0) {
					if (!this.subjectIdParameters.containsKey(subjectAnnotation.paramID())) {
						...
						throw new InconsistentPermissionCheckerEntityException("Duplicate @SubjectID annotation with same patternID "
								+ subjectAnnotation.patternID() + " and paramID " + subjectAnnotation.paramID() + " in method "
								+ m.getName() + " of class " + this.baseClass.getSimpleName());
					}
				}
					...
					if (!this.resourceIdParameters.containsKey(resourceAnnotation.paramID())) {
						...
						throw new InconsistentPermissionCheckerEntityException("Duplicate @ResourceID annotation with same patternID "
					...
					throw new InconsistentPermissionCheckerEntityException("Unidentified parameter at position " + i + " in method "
							+ m.getName() + " in class " + this.baseClass.getSimpleName());
				}
			}
		}
		else {
			throw new InconsistentPermissionCheckerEntityException("Duplicate @CheckAccess method with same patternID "
					+ checkAnnotation.patternID() + " in class " + this.baseClass.getSimpleName());
		}
	}
	...
	public boolean performAccessCheck(HashMap<String, Object> subjectIDs, HashMap<String, Object> resourceIDs, String methodID,
			Object checkerObject) throws InvocationTargetException, IllegalAccessException {
