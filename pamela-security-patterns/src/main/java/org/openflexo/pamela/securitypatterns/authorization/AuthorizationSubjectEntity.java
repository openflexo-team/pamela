...
public class AuthorizationSubjectEntity {
				...
				if (a != null && a.patternID().compareTo(this.pattern.getID()) == 0) {
					if (!paramMapping.containsKey(a.paramID())) {
						...
						throw new ModelDefinitionException(
								String.format("Duplicate ResourceID annotation with same paramID %s in method %s parameters of class %s",
										a.paramID(), method.getName(), this.baseClass.getSimpleName()));
					}
				}
				else {
					realParams.add(i);
	...
	private void processID(Method method, SubjectID annotation) throws InconsistentSubjectEntityException {
		if (!this.idGetters.containsKey(annotation.paramID())) {
			...
			throw new InconsistentSubjectEntityException("Duplicate @SubjectID annotation with same pattern ID (" + this.pattern.getID()
			...
			for (String paramID : this.pattern.getCheckerEntity().getResourceIdParameters().keySet()) {
				for (Method m : this.accessResourceMethods.keySet()) {
					...
					for (Parameter param : m.getParameters()) {
						...
						if (annotation != null && annotation.patternID().compareTo(this.pattern.getID()) == 0
								&& annotation.paramID().compareTo(paramID) == 0) {
							foundInMethod = true;
						}
					}
	...
	public ReturnWrapper processMethodBeforeInvoke(Object self, Method method, Class klass, Object[] args)
		...
		if (this.accessResourceMethods.containsKey(method)) {
			...
			return this.pattern.processAccessResource(subjectIDs, resourceIDs, this.accessResourceMethods.get(method),
					this.methodRealParamIndexes.get(method), self, method, args);
		}
