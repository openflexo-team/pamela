...
public class AuthorizationPattern extends AbstractPattern {
	...
	@Override
	public ReturnWrapper processMethodBeforeInvoke(Object self, Method method, Class klass, Object[] args)
	...
	private void attachResource(Class klass) throws ModelDefinitionException {
		...
		if (resource.isComplete()) {
			...
			throw new InconsistentResourceEntityException(
					"Missing annotations in " + klass.getSimpleName() + "Authorization subject definition with ID " + this.id);
		}
	}
	...
	protected void notifyAccessMethod(String methodID, AuthorizationResourceEntity authorizationResourceEntity)
		...
		if (!this.resourceMethods.containsKey(methodID)) {
			...
			throw new ModelDefinitionException(
					"Duplicate access method with same method id " + methodID + " in pattern " + this.id + " model.");
		}
	}
	...
	protected void attachAuthorizationCheckerFromResource(Class checkerClass) throws InconsistentPermissionCheckerEntityException {
		if (this.checker == null) {
			...
			if (finalKlass == null) {
				throw new InconsistentPermissionCheckerEntityException(
						"Missing annotations in " + checkerClass.getSimpleName() + "Authorization Checker definition with ID " + this.id);
			}
			...
			if (permissionChecker.isComplete()) {
				...
				throw new InconsistentPermissionCheckerEntityException(
						"Missing annotations in " + finalKlass.getSimpleName() + "Authorization Checker definition with ID " + this.id);
			}
		}
	}
	...
	public ReturnWrapper processAccessResource(HashMap<String, Object> subjectIDs, HashMap<String, Object> resourceIDs, String methodID,
			ArrayList<Integer> paramsIndexes, Object self, Method method, Object[] args)
		...
		if (allowed) {
			...
			throw new ModelExecutionException(String.format("Invalid access of subject with ids %s to resource with ids %s using method %s",
					subjectIDs.toString(), resourceIDs.toString(), methodID));
		}
	}
