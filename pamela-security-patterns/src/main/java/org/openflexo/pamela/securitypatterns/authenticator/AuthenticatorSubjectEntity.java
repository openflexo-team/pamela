...
/**
 * This class wraps all the static authenticator pattern related information extracted from parsing a {@link AuthenticatorSubject} annotated
 * class.<br>
 * It has the responsibility of:
 * <ul>
 * <li>Parsing the class.</li>
 * <li>Saving the relevant pattern related information.</li>
 * <li>Discovering and saving at runtime the known instance of the associated class.</li>
 * </ul>
 *
 * @author C. SILVA
 */
...
public class AuthenticatorSubjectEntity {
	 ...
	 * 
	 * @param pattern
	 *            Reference of the associated {@link AuthenticatorPattern}
	 * @param klass
	 *            {@link AuthenticatorSubject} annotated class to analyze
	 * @throws ModelDefinitionException
	 *             When malformed or missing annotation in the subject class or associated authenticator class
	 */
	...
	/**
	 * @return the {@link HashMap} mapping parameters identifier of the {@link AuthenticationInformation} annotated getters with the
	 *         associated getters
	 */
	 ...
	 *             if an error occurred when internally invoking a method
	 */
	ReturnWrapper processMethodBeforeInvoke(Object instance, Method method, Class klass, Object[] args)
	 ...
	 * 
	 * @throws ModelDefinitionException
	 *             In case of inconsistent authenticator or subject definition
	 */
	...
	/**
	 * Saves the <code>Authenticator</code> getter and relays the instantiation of the associated {@link AuthenticatorEntity} to the
	 * pattern.
	 * 
	 * @param method
	 *            Getter of the <code>Authenticator</code>
	 * @param annotation
	 *            {@link AuthenticatorGetter} annotation found on the method
	 * @throws InconsistentAuthenticatorEntityException
	 *             In case of inconsistent authenticator definition (i.e. malformed annotations)
	 */
	 ...
	 * 
	 * @param method
	 *            Getter of an <code>Authentication Information</code>
	 * @param annotation
	 *            {@link AuthenticationInformation} annotation found on the method
	 * @throws InconsistentSubjectEntityException
	 *             In case of malformed {@link AuthenticationInformation} annotation
	 */
	private void processAuthenticationInformation(Method method, AuthenticationInformation annotation)
			throws InconsistentSubjectEntityException {
		if (!this.authInfoGetters.containsKey(annotation.paramID())) {
			this.authInfoGetters.put(annotation.paramID(), method);
		}
		else {
			throw new InconsistentSubjectEntityException("Duplicate @AuthenticationInformation annotation with same pattern ID ("
					+ this.pattern.getID() + ") and paramID (" + annotation.paramID() + ") in " + this.baseClass.getSimpleName());
		}
	}
	 ...
	 * 
	 * @param method
	 *            Setter of the <code>Proof of Identity</code>
	 * @param annotation
	 *            {@link ProofOfIdentitySetter} annotation found on the method
	 * @throws ModelDefinitionException
	 *             In case of malformed annotation or missing getter for <code>Proof of Identity</code>
	 */
	private void processIdProof(Method method, ProofOfIdentitySetter annotation) throws ModelDefinitionException {
		if (this.idProofSetter == null) {
			...
			throw new InconsistentSubjectEntityException("Duplicate @ProofOfIdentity annotation with same pattern ID ("
					+ this.pattern.getID() + ") in " + this.baseClass.getSimpleName());
		}
		Setter setter = method.getAnnotation(Setter.class);
		if (setter == null) {
			throw new InconsistentSubjectEntityException(
					"Proof of identity annotation not on a setter in " + this.baseClass.getSimpleName());
		}
		this.idProofGetter = this.pattern.getContext().getContext().getModelEntity(this.baseClass).getModelProperty(setter.value())
				.getGetterMethod();
	 ...
	 * @param annotation
	 *            {@link AuthenticateMethod} annotation found on the method
	 */
	...
	/**
	 * Attempt to create the ordered list of getter for the <code>Request</code> method parameters of the associated
