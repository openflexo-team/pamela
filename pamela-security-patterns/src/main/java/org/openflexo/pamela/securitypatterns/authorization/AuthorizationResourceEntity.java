...
public class AuthorizationResourceEntity {
	...
	protected boolean isComplete() {
		return this.successLinking && this.baseClass != null && this.pattern != null && this.checkerGetter != null
				&& !this.idGetters.isEmpty() && !this.accessMethods.values().isEmpty();
	}
	...
	private void processCheckerGetter(Method m, PermissionCheckerGetter checkerGetterAnnotation) throws ModelDefinitionException {
		if (this.checkerGetter == null) {
			...
			throw new InconsistentResourceEntityException("Duplicate @PermissionCheckerGetter annotated methods with same patternID "
					+ checkerGetterAnnotation.patternID() + " in class " + this.baseClass.getSimpleName());
		}
	}
	...
	private void processAccessResource(Method method, AccessResource annotation) throws ModelDefinitionException {
		if (!this.accessMethods.containsKey(annotation.methodID())) {
			...
			throw new InconsistentResourceEntityException(
					"Duplicate methodID " + annotation.methodID() + " in class " + this.baseClass.getSimpleName());
		}
	}
	...
	private void processID(Method method, ResourceID annotation) throws InconsistentResourceEntityException {
		if (!this.idGetters.containsKey(annotation.paramID())) {
			...
			throw new InconsistentResourceEntityException("Duplicate @ResourceID annotation with same pattern ID (" + this.pattern.getID()
					+ ") and paramID (" + annotation.paramID() + ") in " + this.baseClass.getSimpleName());
		}
	}
	...
	public ReturnWrapper performAccess(String methodID, Object[] args, ArrayList<Integer> paramsIndexes, Object instance)
